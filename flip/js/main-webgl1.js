...
function app(){
    ...
    const gl = canvas.getContext('webgl', { antialias: false, preserveDrawingBuffer: true, premultipliedAlpha: false, alpha: true });
    ...
    function start(){
        ...
        for (let i=0; i<MAX_NUM; i++){
            ...
            //off[i] = data[0];//Math.floor(Math.random() * 80); //random number for character
        ...
        const fs = `
            ...
            void main() {
                ...
                vec4 col = texture2D(spriteTexture, gl_PointCoord / vec2(32.0, 1.0) + (vec2(idx, idy) / vec2(256.0, 8.0)));
                ...
                //gl_FragColor = vec4(x / 256.0, 0.0, y / 32.0, 1.0);
                gl_FragColor = col;
            }
        `;
    ...
    function loop(time) {
        ...
        if (fps.delta > fps.interval) {
            ...
            // ctx.beginPath();
            // ctx.arc(32+Math.cos(fps.loopTime+30) * 16, 32+Math.sin(fps.loopTime+30) * 16, Math.abs(Math.sin(fps.loopTime+15) * 16), 0, 2 * Math.PI);
            // ctx.fill();
            ...
            for (let i = 0; i < MAX_NUM; i++){
                ...
                points[i*3 + 1] = y * 16 + 8;
                points[i*3 + 2] = flaps[i].out;

                //flaps[i].setDelay(i % 9);
            ...
            if(fps.frameStep % 1 == 0) {
                ...
                for (let i = 0; i < MAX_NUM; i++){
                    ...
                    //flaps[i].setDelay(i % 5);
                    //flaps[i].setDelay(0);
                    //flap.setTarget(Math.floor(Math.random()*255));
                    //if (toggle == false) {
                    flaps[i].setTarget(off[i]);
                    // } else {
                    //    flaps[i].setTarget(off[i]);
